...
기존 Redux/Zustand DevTools가 상태 변화를 "관찰"하는 데 그친다면, Z-UI는 GUI에서 상태를 직접 편집하고, 스냅샷을 저장/복원하고, 스토어 간 의존성을 그래프로 그려보는 등 **능동적으로 상태 로직을 설계·디버깅**하는 걸 목표로 합니다.
...
자세한 아키텍처는 [ROADMAP.md](./ROADMAP.md)를 참고하세요.
...
아직 npm 배포 전이므로, 저장소를 클론해서 워크스페이스로 실행합니다.
